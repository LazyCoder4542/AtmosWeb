    ...
    for (var i = 0; i < particles.length; i++) {
      ...
      if (!p.frozen) {
        ...
        if (p.x > window.innerWidth || p.x < 0) {
          if ((p.x > window.innerWidth && Math.sign(p.vx) == 1) || (p.x < window.innerWidth && Math.sign(p.vx) == -1)) {
            p.vx = -p.vx
            p.x += p.vx;
          }
        } else if (p.y > window.innerHeight || p.y < 0) {
          if ((p.y > window.innerHeight && Math.sign(p.vy) == 1) || (p.y < window.innerHeight && Math.sign(p.vy) == -1)) {
            p.vy = -p.vy
            p.y += p.vy;
          }
        }
